...
class CamInterface:
    def __init__(self, _camera_id="/dev/video0", cam_format='v4l2', options=None):
        if options is None:
            options = {}
        self._camera = av.open(file=env("VID_CAMERA_DEVICE", default=_camera_id, cast=str),
                               format=env("VID_CAMERA_FORMAT", default=cam_format, cast=str),
                               options=options)
